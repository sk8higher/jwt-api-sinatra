...
class Api < Sinatra::Base
  ...
  def access_token(user_id)
    JWT.encode(payload(user_id, 'access'), ENV['JWT_SECRET'], 'HS512')
  end

  def refresh_token(user_id)
    if @user.refresh_token.empty?
      @user.refresh_token = BCrypt::Password.create(JWT.encode(payload(user_id, 'refresh'), ENV['JWT_SECRET'], 'HS512'))
      @user.save
    end
    @user.refresh_token
  end

  def payload(user_id, type)
    {
